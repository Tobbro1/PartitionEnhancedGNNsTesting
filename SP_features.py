...
class SP_graph_features():
    ...
    def __init__(self, num_samples: int, label_alphabet: list[int], distances_alphabet: list[int]):
        super().__init__()
        ...
        self.editmask = np.full((len(label_alphabet)**2) * len(distances_alphabet), False)
        self.dataset = np.full((num_samples, (len(label_alphabet)**2) * len(distances_alphabet)), 0)

    def edit_dataset_by_index(self, idx: int, vector: np.array):

        assert vector.shape == self.dataset[0,:].shape
        assert 0 <= idx and idx < self.dataset[:,0].shape[0]

        self.dataset[idx,:] = vector
    ...
    def floyd_warshall(self, graph: Data) -> np.array:
        ...
        _graphnx = torch_geometric.utils.to_networkx(graph)

        return nx.floyd_warshall_numpy(_graphnx)
    ...
    def sp_feature_vector_from_feature_map(self, dict: Dict[Tuple[Tuple[int, int], int], int], idx: int) -> np.array:

        result = np.zeros((len(self.label_alphabet)**2) * len(self.distances_alphabet))
        ...
        self.edit_dataset_by_index(idx = idx, vector = result)
        return result
...
def get_file_from_local_by_path(root_path, filename):
...
sp_feature_generator = SP_graph_features(num_samples = 2, label_alphabet=range(3), distances_alphabet=range(1,12))
...
vector = sp_feature_generator.sp_feature_vector_from_feature_map(dict = _sp_map, idx = 0)
print('Feature vector 1: ' + str(vector))
vector2 = sp_feature_generator.sp_feature_vector_from_feature_map(dict = _sp_map2, idx = 1)
print('Feature vector 2: ' + str(vector2))

root_path = osp.join('data', 'SP_features')

print('Writing the uncropped dataset..')
sp_feature_generator.write_dataset(path=get_file_from_local_by_path(root_path = root_path, filename = 'SP_features.svmlight'), comment='Uncropped SP features')
print('Writing the cropped dataset..')
sp_feature_generator.crop_feature_vectors()
print('Cropped Feature vector 1: ' + str(sp_feature_generator.get_dataset()[0,:]))
print('Cropped Feature vector 2: ' + str(sp_feature_generator.get_dataset()[1,:]))
sp_feature_generator.write_dataset(path=get_file_from_local_by_path(root_path = root_path, filename = 'SP_features_cropped.svmlight'), comment='Cropped SP features')
